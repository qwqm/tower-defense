...
export class Game {
  ...
  /** 相邻将魂按左→右（池内按槽位）排序后判定；逆序不会觉醒。 */
  heroForTokenPair(a: Unit, b: Unit): HeroKey | null {
    if (a.loc[0].t !== b.loc[0].t) return null;
    const [first, second] = [a, b].sort((x, y) => x.loc[0].i - y.loc[0].i);
    const adjacent = first.loc[0].t === 1
