...
export function Screen({ title, onBack, children, right }: { title: string; onBack: () => void; children: ReactNode; right?: ReactNode }) {
  return (
    <div className="ink-noise ink-vignette relative flex h-full flex-col overflow-hidden bg-[#f2ebdc]">
      <div className="relative z-10 flex items-center gap-3 border-b border-[#3b3229]/15 bg-[#efe6d2]/82 px-4 py-3 shadow-[0_4px_16px_rgba(60,45,25,0.08)] backdrop-blur-md">
        <button onClick={() => { sfx('click'); onBack(); }} className="rounded-lg border border-[#3b3229]/20 bg-[#3b3229]/10 px-3 py-1.5 text-[#3b3229] transition hover:bg-[#3b3229]/15 active:scale-95">返回</button>
        <div className="ink-title ink-reveal text-xl text-[#2c251d]">{title}</div>
        <div className="ml-auto">{right}</div>
      </div>
      <div className="relative z-10 min-h-0 flex-1 overflow-y-auto px-4 py-4">{children}</div>
    </div>
  );
}

export function Card({ children, className = '' }: { children: ReactNode; className?: string }) {
  return <div className={`ink-noise relative overflow-hidden rounded-2xl border border-[#3b3229]/15 bg-[#fbf6e9]/92 p-3 shadow-[0_5px_16px_rgba(60,45,25,0.09)] ${className}`}>{children}</div>;
}
